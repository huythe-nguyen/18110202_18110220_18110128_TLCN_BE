module.exports.authController = require('./admin/auth.controller')
module.exports.productController = require('./admin/product.controller')
module.exports.brandController = require('./admin/brand.controller')
module.exports.newController = require('./admin/new.controller')
module.exports.userController = require('./admin/user.controller')
module.exports.oderController = require('./admin/oder.controller')
module.exports.cartController = require('./admin/cart.controller')
module.exports.discountController = require('./admin/discount.controller')
module.exports.reviewsController = require('./admin/reviews.controller')


module.exports.authControllerUser = require('./client/auth-controller')
module.exports.productControllerUser = require('./client/product-controller')
module.exports.cartControllerUser = require('./client/cart.controller')
