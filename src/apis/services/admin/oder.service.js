...
const viewState = async (page, size, key, daystart, dayend) => {
        var today = moment(daystart).startOf('day');
        // "2018-12-05T00:00:00.00
        var tomorrow = moment(dayend).endOf('day');
        // ("2018-12-05T23:59:59.999
        if (page) {
            pages = parseInt(page);
            if (pages < 1)
...
const counts = async (key,day) => {
    ...
    if(days<=0){
        ...
        var tomorrow = moment(today).endOf('day');
        // ("2018-12-05T23:59:59.999
        const list = await Cart.find({ state: key, updatedAt: { '$gte': today, '$lte': tomorrow } }).count()
        return list
    }else{
        var today = moment().subtract(days,'day');
        // "2018-12-05T00:00:00.00
